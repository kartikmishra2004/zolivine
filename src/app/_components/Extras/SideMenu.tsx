...
export default function SideMenu({ setMenuOpen, menuOpen }: Props) {
    ...
    return (
        <>
            <section onClick={() => setMenuOpen(false)} className={`w-full h-screen bg-black/40 z-[9999] ${menuOpen ? "fixed" : "hidden"} transition-all duration-700 ease-initial ${menuOpen ? "opacity-100" : "opacity-0"}`}></section>
